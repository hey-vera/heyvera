...
import { getCheapBrandIntelForChat, logXIntelMeasurement, searchKnowledge, upsertKnowledgeToGateway, type KnowledgeItem, type IntelMeta } from "../../src/core/x-intel-gateway.js";
import { getGitHubContextForChat } from "../../src/core/github-intel.js";
...
export async function handleChatMessage(
  ...
  actions?: ToolAction[];
  usage?: LLMUsage;
  intel?: { x: any; knowledge: any } | null;
}> {
  ...
  // Use tenantId (from server route) as primary brand key for intel (matches push + server tenant.id).
  const currentBrand = tenantId || context?.brandName || currentHostedRuntimeAgentId() || 'brand';

  // Extracted pure assembler (shipped, testable). Returns blocks + precomputed note + meta.
  // No catch here: errors surface for observability (intel path must not swallow).
  const assembled = await assembleChatIntelContext(currentBrand, userMessage);
  const xIntelBlock = assembled.xIntelBlock;
  const ghBlock = assembled.ghBlock;
  const knowledgeBlock = assembled.knowledgeBlock;
  const xIntelMeta: IntelMeta | null = assembled.xIntelMeta;
  const knowledgeMeta: IntelMeta | null = assembled.knowledgeMeta;
  const intelNote = assembled.intelNote;

  const fullSystemPrompt = SYSTEM_PROMPT + contextBlock + xIntelBlock + ghBlock + knowledgeBlock;
  ...
  if (!llmResult) {
    // Always surface intel + note even if LLM failed (AC3). Use precomputed note.
    const fallbackReply = "Sorry, I'm having trouble connecting. Try again in a moment." + (intelNote || '');
    return {
      reply: fallbackReply,
      configReady: false,
      intel: {
        x: xIntelMeta ? { cost: xIntelMeta.data_cost_usdc, savings: xIntelMeta.savings_usdc, trace: xIntelMeta.decision_trace } : null,
        knowledge: knowledgeMeta ? { cost: knowledgeMeta.data_cost_usdc, savings: knowledgeMeta.savings_usdc, trace: knowledgeMeta.decision_trace } : null,
      },
    };
  }
  ...
  // Use precomputed intelNote from assembler (AC3) — even on LLM fail path below.
  const finalReply = cleanReply + (intelNote || '');
  session.messages.push({ role: "assistant", content: finalReply });
  saveMessage(session.conversationId, "assistant", finalReply);
  ...
  return {
    ...
    usage,
    // unified intel observability for end-to-end
    intel: {
      x: xIntelMeta ? { cost: xIntelMeta.data_cost_usdc, savings: xIntelMeta.savings_usdc, trace: xIntelMeta.decision_trace } : null,
      knowledge: knowledgeMeta ? { cost: knowledgeMeta.data_cost_usdc, savings: knowledgeMeta.savings_usdc, trace: knowledgeMeta.decision_trace } : null,
    },
