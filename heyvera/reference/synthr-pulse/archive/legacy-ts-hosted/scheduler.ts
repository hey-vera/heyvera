...
export async function runAgentTasks(
  ...
  try {
    ...
    console.error('[scheduler] buildGitHubContextBlock failed', e);
    // surface for intel path
    throw e;
  }

  // Exercise scheduler path for semantic GitHub/Knowledge intel (from gateway) - no swallow, let error surface if any
  const { searchKnowledge } = await import("../src/core/x-intel-gateway.js");
  const k = searchKnowledge(tenant.id, 'project context', 2);
  if (k.items?.length > 0) {
    console.log(`[Scheduler] GitHub intel available via gateway for ${tenant.id}: ${k.items.length} hits, cost $${k.meta.data_cost_usdc}`);
  }
