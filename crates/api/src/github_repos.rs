...
async fn run_import(
    ...
    let Some(cm) = state.container_manager.as_ref() else {
        update_progress(&state, &import_id, "failed", 0, "container runtime unavailable", Some("container runtime unavailable"));
        return;
    };
    ...
    update_progress(&state, &import_id, "importing", 10, "provisioning container", None);
    ...
    let container_id = match cm.ensure_container(db, &user_id, IMPORT_PROVIDER).await {
        ...
        Err(e) => {
            ...
            update_progress(&state, &import_id, "failed", 10, "container provisioning failed", Some(&e));
            return;
        }
    };
    db.touch_container_activity(&user_id);

    update_progress(&state, &import_id, "importing", 30, "cloning repository", None);

    // Ensure the repos root exists and is owned by the sandbox user.
    let _ = cm
        .exec_in_container(&container_id, &["mkdir", "-p", REPOS_ROOT], None, GIT_OP_TIMEOUT)
        .await;

    // Fresh clone: if the directory already exists (re-import), remove it first
    // so the clone starts clean.
    let _ = cm
        .exec_in_container(&container_id, &["rm", "-rf", &clone_path], None, GIT_OP_TIMEOUT)
        .await;

    let extraheader = auth_extraheader(&token);
    let clone_url = format!("https://github.com/{repo_full_name}.git");
    let clone_cmd = vec![
        "git", "-c", extraheader.as_str(),
        "clone", "--depth", "50", "--branch", default_branch.as_str(),
        clone_url.as_str(), clone_path.as_str(),
    ];
    ...
    match clone_res {
        ...
        Ok(r) => {
            ...
            tracing::warn!(user_id, import_id, exit = r.exit_code, "github import: clone failed");
            update_progress(&state, &import_id, "failed", 30, "clone failed", Some(&detail));
            return;
        }
        Err(e) => {
    ...
    update_progress(&state, &import_id, "importing", 80, "configuring repository", None);

    // Identify Cortex as the committer for any future sync commits.
    let _ = cm.exec_in_container(&container_id, &["git", "-C", &clone_path, "config", "user.name", "Cortex"], None, GIT_OP_TIMEOUT).await;
    let _ = cm.exec_in_container(&container_id, &["git", "-C", &clone_path, "config", "user.email", "bot@cortex.heyvera.ai"], None, GIT_OP_TIMEOUT).await;

    // Capture the cloned HEAD commit for provenance.
    let head_commit = cm
        .exec_in_container(&container_id, &["git", "-C", &clone_path, "rev-parse", "HEAD"], None, GIT_OP_TIMEOUT)
        .await
        .ok()
    ...
    db.audit_log(
        &user_id, "user", "github.import.completed",
        Some("github_import"), Some(&import_id),
        Some(&format!("{{\"repo\":\"{repo_full_name}\"}}")), None,
    );
...
pub async fn sync_repo(
    ...
    let cm = state
        .container_manager
        .as_ref()
        .ok_or_else(|| err(StatusCode::SERVICE_UNAVAILABLE, "container runtime not available"))?;
    ...
    let status_out = cm
        .exec_in_container(&container_id, &["git", "-C", path, "status", "--porcelain"], None, GIT_OP_TIMEOUT)
        .await
        .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e))?;

    if !status_out.stdout.trim().is_empty() {
        let _ = cm
            .exec_in_container(&container_id, &["git", "-C", path, "add", "-A"], None, GIT_OP_TIMEOUT)
            .await;
        let msg = req
            .message
            .clone()
        ...
        let commit = cm
            .exec_in_container(&container_id, &["git", "-C", path, "commit", "-m", &msg], None, GIT_OP_TIMEOUT)
            .await
            .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e))?;
    ...
    let pull = cm
        .exec_in_container(
            &container_id,
            &["git", "-c", &extraheader, "-C", path, "pull", "--rebase", "origin", &imp.default_branch],
            None,
            GIT_OP_TIMEOUT,
        )
        .await
    ...
    if pull.exit_code != 0 {
        ...
        if combined.contains("conflict") || combined.contains("rebase") {
            let conflicts = cm
                .exec_in_container(&container_id, &["git", "-C", path, "diff", "--name-only", "--diff-filter=U"], None, GIT_OP_TIMEOUT)
                .await
                .ok()
                .map(|r| r.stdout.trim().to_string())
                .unwrap_or_default();
            let _ = cm
                .exec_in_container(&container_id, &["git", "-C", path, "rebase", "--abort"], None, GIT_OP_TIMEOUT)
                .await;

            db.audit_log(
                &user.user_id, "user", "github.sync.conflict",
                Some("github_import"), Some(&import_id), None, None,
            );
    ...
    if req.push {
        let push = cm
            .exec_in_container(
                &container_id,
                &["git", "-c", &extraheader, "-C", path, "push", "origin", &imp.default_branch],
                None,
                GIT_OP_TIMEOUT,
            )
            .await
    ...
    let head_commit = cm
        .exec_in_container(&container_id, &["git", "-C", path, "rev-parse", "HEAD"], None, GIT_OP_TIMEOUT)
        .await
        .ok()
        .filter(|r| r.exit_code == 0)
        .map(|r| r.stdout.trim().to_string());
    ...
    db.mark_github_import_synced(&import_id, head_commit.as_deref());
    db.audit_log(
        &user.user_id, "user", "github.sync.completed",
        Some("github_import"), Some(&import_id), None, None,
    );
...
mod tests {
    ...
    fn extraheader_is_basic_auth() {
        ...
        assert_eq!(String::from_utf8(decoded).unwrap(), "x-access-token:secrettoken");
    }
