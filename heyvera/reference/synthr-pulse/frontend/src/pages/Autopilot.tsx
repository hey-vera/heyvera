...
type GoalExecution = {
  ...
  steps: Array<{ id: string; description: string; status: string; output?: any }>
  costMeta?: { totalUsd?: number; items?: Array<any> }
  result?: { draft?: { text?: string } }
...
export default function Autopilot() {
  ...
  const [goalText, setGoalText] = useState('Create one useful X post from recent audience signals')
  const [goalRun, setGoalRun] = useState<GoalExecution | null>(null)
  ...
  useEffect(() => {
    if (!goalRun?.id || ['completed', 'failed', 'waiting_approval'].includes(goalRun.status)) return
    const timer = setInterval(async () => {
      try {
        const data = await get<{ goal: GoalExecution }>(`/v1/goal/${goalRun.id}/status`)
        setGoalRun(data.goal)
      } catch {
        setGoalStatus('Could not refresh goal status.')
      }
    }, 1500)
    return () => clearInterval(timer)
  }, [goalRun?.id, goalRun?.status])

  const startDurableGoal = async () => {
    const goal = goalText.trim()
    if (!goal || startingGoal || (balanceUsd != null && balanceUsd <= 0)) return
    setStartingGoal(true)
    setGoalStatus(null)
    try {
      const data = await post<{ goal: GoalExecution }>('/v1/goal/start', {
        brandId: activeBrandId,
        goal,
        approvalRequired: true,
      })
      setGoalRun(data.goal)
      setGoalStatus('Goal started.')
    } catch (err: any) {
      setGoalStatus(`Failed: ${err?.message || 'Could not start goal.'}`)
    } finally {
  ...
  const runAction = async (action: 'post' | 'reply') => {
    if (activeAction || (balanceUsd != null && balanceUsd <= 0)) return
    ...
    try {
      if (action === 'post') {
        ...
        if (mode === 'full') {
          ...
          if (d.ok && (d.repliedCount > 0 || d.likedCount > 0)) {
            ...
            // Show draft replies with "Reply on X" links (for Free tier users)
            if (d.drafts?.length > 0) {
              ...
              msg += '<div style="font-size:10px;color:var(--text-4);margin-top:4px;">X Free tier limits auto-replies. Upgrade to X Basic ($100/mo) for full automation.</div>'
              msg += '</div>'
            }
      ...
      setPostNowResult(`Failed: ${err?.message || 'Unknown error. Check balance and X connection.'}`)
    }
    ...
    <div style={{ width: '100%', maxWidth: 720, margin: '0 auto', padding: '0 24px', overflowX: 'hidden', boxSizing: 'border-box', wordBreak: 'break-word', minWidth: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-3)', margin: 0 }}>Control how your agent posts</p>
          </div>
        </div>
      ...
      {isNewAgent && (
        <div style={{
          ...
          <div style={{ fontSize: 12, color: 'var(--text-3)', lineHeight: 1.5 }}>
            ...
            You can start using your agent now — content quality improves as research completes.
          </div>
      ...
      <div style={{
        ...
        {isOff ? (
          ...
          <div>
            ...
            {/* Warning: autopilot on but agent not running */}
            {!agentRunning && (
              <div style={{
                ...
                <span style={{ fontSize: 12, color: '#f59e0b', lineHeight: 1.4 }}>
                  Autopilot is configured but your agent is paused. <Link to="/settings" style={{ color: '#f59e0b', fontWeight: 600 }}>Start it in Settings</Link> to begin posting.
                </span>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              {([
                ...
                <button
                  ...
                  disabled={!!activeAction || (balanceUsd != null && balanceUsd <= 0)}
                  style={{
                    ...
                    cursor: activeAction || (balanceUsd != null && balanceUsd <= 0) ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                    opacity: (activeAction && activeAction !== a.key) || (balanceUsd != null && balanceUsd <= 0) ? 0.4 : 1,
            ...
            {balanceUsd != null && balanceUsd <= 0 && (
              <div style={{
                ...
                Your balance is $0.00. Add funds to resume agent actions.
              </div>
            )}
      ...
      <div style={{ ...cardStyle, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-1)' }}>Goal Runner</div>
            <div style={{ fontSize: 12, color: 'var(--text-4)', marginTop: 2 }}>
              {goalRun?.temporalWorkflowId || 'Temporal-ready execution'}
            </div>
          </div>
          {goalRun && (
            <span style={{
              padding: '4px 8px', borderRadius: 999, fontSize: 11, fontWeight: 600,
              color: goalRun.status === 'waiting_approval' ? '#f59e0b' : 'var(--accent)',
              background: goalRun.status === 'waiting_approval' ? 'rgba(245,158,11,0.1)' : 'rgba(16,185,129,0.1)',
              border: goalRun.status === 'waiting_approval' ? '1px solid rgba(245,158,11,0.2)' : '1px solid rgba(16,185,129,0.2)',
            }}>{goalRun.status.replace('_', ' ')}</span>
          )}
        </div>
        <textarea
          value={goalText}
          ...
          rows={3}
          style={{ ...inputStyle, minHeight: 82, resize: 'vertical', lineHeight: 1.4, marginBottom: 10 }}
        />
        <button
          onClick={startDurableGoal}
          disabled={startingGoal || !goalText.trim() || (balanceUsd != null && balanceUsd <= 0)}
          style={{
            width: '100%', padding: '10px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600,
            background: startingGoal ? 'var(--accent)' : 'var(--bg-2)',
            color: startingGoal ? '#fff' : 'var(--text-2)',
            border: startingGoal ? '1px solid var(--accent)' : '1px solid var(--border)',
            cursor: startingGoal || (balanceUsd != null && balanceUsd <= 0) ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
            opacity: startingGoal || (balanceUsd != null && balanceUsd <= 0) ? 0.6 : 1,
          }}
        >
          {startingGoal && <Loader2 size={13} style={{ animation: 'spin 1s linear infinite' }} />}
          {startingGoal ? 'Starting...' : 'Start Goal'}
        </button>
        {goalStatus && (
          <div style={{ marginTop: 8, fontSize: 12, color: goalStatus.startsWith('Failed') ? 'var(--danger)' : 'var(--text-3)' }}>
            {goalStatus}
          </div>
        )}
        {goalRun && (
          <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {goalRun.steps.map(step => (
              <div key={step.id} style={{
                ...
                <span style={{
                  color: step.status === 'completed' ? 'var(--accent)' : step.status === 'waiting_approval' ? '#f59e0b' : 'var(--text-4)',
                  fontWeight: 600,
                }}>{step.status.replace('_', ' ')}</span>
                <span style={{ color: 'var(--text-2)', minWidth: 0 }}>{step.description}</span>
              </div>
            ))}
            {typeof goalRun.costMeta?.totalUsd === 'number' && (
              <div style={{ fontSize: 11, color: 'var(--text-4)' }}>
                X intel cost: ${goalRun.costMeta.totalUsd.toFixed(4)}
              </div>
            )}
            {goalRun.result?.draft?.text && (
              <div style={{ padding: 10, borderRadius: 8, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', fontSize: 12, color: 'var(--text-2)', lineHeight: 1.5 }}>
                {goalRun.result.draft.text}
              </div>
            )}
          </div>
      ...
      {replyDrafts.length > 0 && (
        <div style={{ ...cardStyle, marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--text-4)', marginTop: 8 }}>
            X Free tier limits auto-replies. These drafts are ready to post manually. Upgrade to X Basic ($100/mo) for full automation.
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <Section title="Daily Limits" icon={Shield}>
          <p style={{ fontSize: 12, color: 'var(--text-4)', margin: '0 0 14px', lineHeight: 1.4 }}>Hard caps — your agent will never exceed these, even in Full Auto.</p>
      ...
      <Modal
        ...
        message="Full Auto mode will post to your connected accounts autonomously. Your agent's personality comes from your knowledge notes and brand settings."
