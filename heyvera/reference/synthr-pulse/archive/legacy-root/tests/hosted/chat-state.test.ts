...
describe("chat state", () => {
  it("preserves message ordering and stores assistant replies without tool tags", async () => {
    ...
    // Note: shipped handler now appends intel usage note to replies (desired for observability)
    const rows = messageRows(conversation.id);
    expect(rows[0]).toEqual({ role: "user", content: "first user message" });
    expect(rows[1].role).toBe("assistant");
    expect(rows[1].content).toContain("First answer.");
    expect(rows[1].content).toContain("[used X + GitHub intel");
  });
  ...
  it("returns export actions while persisting a clean assistant reply", async () => {
    ...
    const [conversation] = activeConversationRows();
    const exportRows = messageRows(conversation.id);
    expect(exportRows[0]).toEqual({ role: "user", content: "export my setup" });
    expect(exportRows[1].content).toContain("Your profile is ready.");
    expect(exportRows[1].content).toContain("[used X + GitHub intel");
  });
  ...
  it("archives active chat on reset and starts a fresh conversation", async () => {
    ...
    const resetRows = messageRows(conversations[0].id).map((row) => row.content);
    expect(resetRows[0]).toBe("before reset");
    expect(resetRows[1]).toContain("Before reset.");
    expect(resetRows[1]).toContain("[used X + GitHub intel");
    const afterReset = messageRows(conversations[1].id).map((row) => row.content);
    expect(afterReset[0]).toBe("after reset");
    expect(afterReset[1]).toContain("After reset.");
    expect(afterReset[1]).toContain("[used X + GitHub intel");
  });
  ...
  it("keeps self-host data-dir chat reset scoped to the active agent", async () => {
    ...
    const agentRows = messageRows(conversations[1].id).map((row) => row.content);
    expect(agentRows[0]).toBe("agent b message");
    expect(agentRows[1]).toContain("Agent B answer.");
    expect(agentRows[1]).toContain("[used X + GitHub intel");
    const freshRows = messageRows(conversations[2].id).map((row) => row.content);
    expect(freshRows[0]).toBe("agent a fresh message");
    expect(freshRows[1]).toContain("Agent A fresh answer.");
    expect(freshRows[1]).toContain("[used X + GitHub intel");
  });
});
