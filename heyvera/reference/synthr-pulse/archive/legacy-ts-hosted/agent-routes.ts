...
// ─── POST /v1/pulse/intel/research (thin deep_research primitive) ─────────────
// Uses real shipped assembleChatIntelContext + knowledge/github search.
// Returns structured result + IntelMeta (cost/savings/trace). x402 verified.
agentRouter.post("/intel/research", async (c) => {
  const tenant = c.get("agentTenant") || {};
  let body: any;
  try {
    body = await c.req.json();
  ...
  const { query, brandId } = body || {};
  const effectiveBrand = brandId || tenant.id || 'x402-default';
  ...
  const paid = await verifyX402Payment(c, cost);
  if (!paid) {
    return await createX402Response(c, cost);
  }
  return withTenantContext(effectiveBrand, async () => {
    const { result, meta } = await runIntelResearch({ query, brandId: effectiveBrand, paidCostUsd: cost });
    return c.json({ ok: true, result, meta, paidVia: "x402" });
  });
});

// ─── POST /v1/pulse/goal/decompose (thin decompose_goal primitive) ────────────
// Exercises real decomposeGoal from core (planner/worker contract model).
// Returns GoalPlan + meta.
agentRouter.post("/goal/decompose", async (c) => {
  const tenant = c.get("agentTenant") || {};
  let body: any;
  try {
    body = await c.req.json();
  ...
  const { goal, brandId } = body || {};
  const effectiveBrand = brandId || tenant.id || 'x402-default';
  if (!goal || typeof goal !== "string") {
    return c.json({ error: "goal is required", code: "INVALID_INPUT" }, 400);
  }
  const cost = 0.0005;
  const paid = await verifyX402Payment(c, cost);
  if (!paid) {
  ...
  return withTenantContext(effectiveBrand, async () => {
    const { result, meta } = runGoalDecompose({ goal, brandId: effectiveBrand, paidCostUsd: cost });
    return c.json({ ok: true, result, meta, paidVia: "x402" });
  });
});
