...
// Register in src/index.ts: app.route('/', contactRoute)
// Requires: RESEND_API_KEY, RESEND_FROM, ADMIN_EMAIL in .env
...
contact.post(
  ...
  async (c) => {
    ...
    if (data.website && data.website.length > 0) {
      ...
      return c.json({ ok: true, message: 'Message received.' }) // fake success
    }
