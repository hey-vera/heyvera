...
export async function syncGitHubRepoContext(
  ...
  // Verif seam: allow synthetic snapshot for gateway-sync evidence without net/token (drives syncAll path)
  if (opts.fullName === 'synthetic/verif' || process.env.PULSE_VERIF_SYNTH_GITHUB === '1') {
    return {
      repoId: opts.repoId,
      fullName: opts.fullName,
      trustMode: opts.trustMode,
      generatedAt: new Date().toISOString(),
      summary: 'synthetic verif repo for intel gateway sync log',
      metadata: { description: 'synthetic verif', language: '', topics: [], stars: 0, defaultBranch: 'main' },
      readme: 'Synthetic README with github sync keywords.',
      commits: [{ sha: 'abc1234', message: 'test commit', author: 'verif', date: '' }],
      prs: [{ number: 42, title: 'test PR', body: '' }],
...
// After sync, push key content to the unified intel gateway for semantic use in chat/research.
// This makes GitHub part of the 10/10 automated intelligence with cache hits and cost tracking.
// Always produces at least a summary item (for metadata trustMode too). Includes commits/prs/fileTree when present (AC1).
export async function pushGitHubToIntelGateway(tenantId: string, snapshot: GitHubRepoSnapshot) {
  try {
    const items: Array<{ id: string; source: string; content: string; metadata: Record<string, any> }> = [];

    // Always include summary + metadata for *any* trustMode (so metadata sync yields retrievable intel)
    const summaryContent = `${snapshot.summary}\n${JSON.stringify(snapshot.metadata || {})}`;
    items.push({
      id: `${snapshot.repoId}:summary`,
      source: `github:${snapshot.fullName}`,
      content: summaryContent.slice(0, 4000),
      metadata: { type: 'summary', trustMode: snapshot.trustMode, brand_id: tenantId, ... (snapshot.metadata || {}) },
    });

    if (snapshot.readme) {
      items.push({
        id: `${snapshot.repoId}:readme`,
        source: `github:${snapshot.fullName}`,
        content: snapshot.readme.slice(0, 8000),
        metadata: { type: 'readme', trustMode: snapshot.trustMode, brand_id: tenantId },
      });
    }
    if (snapshot.files) {
      for (const f of snapshot.files.slice(0, 5)) {
        items.push({
          id: `${snapshot.repoId}:${f.path}`,
          source: `github:${snapshot.fullName}`,
          content: `${f.path}:\n${f.content.slice(0, 2000)}`,
          metadata: { type: 'file', path: f.path, trustMode: snapshot.trustMode, brand_id: tenantId },
        });
      }
    }
    if (snapshot.commits && snapshot.commits.length) {
      for (const c of snapshot.commits.slice(0, 3)) {
        items.push({
          id: `${snapshot.repoId}:commit:${c.sha}`,
          source: `github:${snapshot.fullName}`,
          content: `commit ${c.sha} by ${c.author}: ${c.message}`,
          metadata: { type: 'commit', sha: c.sha, trustMode: snapshot.trustMode, brand_id: tenantId },
        });
      }
    }
    if (snapshot.prs && snapshot.prs.length) {
      for (const p of snapshot.prs.slice(0, 2)) {
        items.push({
          id: `${snapshot.repoId}:pr:${p.number}`,
          source: `github:${snapshot.fullName}`,
          content: `PR #${p.number} ${p.title}\n${(p.body || '').slice(0, 500)}`,
          metadata: { type: 'pr', number: p.number, trustMode: snapshot.trustMode, brand_id: tenantId },
        });
      }
    }
    if (snapshot.fileTree && snapshot.fileTree.length) {
      items.push({
        id: `${snapshot.repoId}:filetree`,
        source: `github:${snapshot.fullName}`,
        content: `File tree: ${snapshot.fileTree.slice(0, 30).join(', ')}`,
        metadata: { type: 'filetree', trustMode: snapshot.trustMode, brand_id: tenantId },
      });
    }

    if (items.length > 0) {
      const { upsertKnowledgeToGateway } = await import('../src/core/x-intel-gateway.js');
      upsertKnowledgeToGateway(items);
    }
  } catch (e) {
    console.error('[pushGitHubToIntelGateway] error (logged, not swallowed silently)', e);
    throw e;  // surface for callers that care
  }
}
...
export async function syncAllLinkedRepos(
  ...
  for (const link of links) {
    try {
      ...
      // Push must succeed for this repo to be considered synced; failure rethrows to outer catch
      // (repo logged failed, not "synced" — prevents intel missing post-sync)
      await pushGitHubToIntelGateway(tenantId, snapshot);
