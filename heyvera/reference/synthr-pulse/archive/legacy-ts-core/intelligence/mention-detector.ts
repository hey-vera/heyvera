...
import { getXIntelMentions, logXIntelMeasurement } from '../core/x-intel-gateway.js';
import {
  mentionReplyDelay,
  buildVoiceBlock,
  buildPersonContext,
...
export async function detectMentions(): Promise<DetectedMention[]> {
  ...
  for (const q of queries) {
    ...
    try {
      let resultsForMentions: Array<{ url: string; text: string; author?: string }> = [];

      if (q.platform === 'x' || q.site.includes('x.com')) {
        // === NEW 10/10 PATH: Use Rust ClawAPIsXRouter gateway when available ===
        // Delivers native X data, semantic/exact cache, and real cost/savings metadata.
        const intel = await getXIntelMentions(config.persona.brandName || brandName || 'brand', q.query, {
          purpose: 'mention_monitor',
          sinceHours: 24,
        });

        // Log the first measurement — this is how we prove affordability + quality at scale.
        logXIntelMeasurement(intel.meta, `mentions:${q.query}`);

        // Convert high-quality XPost results into the shape the rest of the pipeline expects.
        resultsForMentions = intel.posts.map(p => ({
          url: p.url,
          text: p.text,
          author: p.author_handle,
        }));

        // If the gateway gave us great data, great. Otherwise fall through to legacy search below.
        if (resultsForMentions.length > 0) {
          for (const r of resultsForMentions) {
            const id = extractMentionId(r.url, q.platform) || `gw_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
            if (state.processedIds.includes(id)) continue;
            if (blocklist.some(b => b.toLowerCase() === (r.author || '').toLowerCase())) continue;
            ...
            const mention: DetectedMention = {
              ...
              platform: q.platform,
              url: r.url,
              author: r.author || extractAuthor(r.url, q.platform),
              text: r.text,
              sentiment: 'neutral',
              detectedAt: new Date().toISOString(),
              replyAfter: new Date(Date.now() + delay).toISOString(),
              suggestedReply: null,
              status: 'pending',
            };
          ...
          continue; // Used the modern gateway path
        }
      }

      // Legacy fallback (Serper / other web search with site:)
