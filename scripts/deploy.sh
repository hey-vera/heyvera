...
GITHUB_DEPLOY_RUN_ID="${GITHUB_DEPLOY_RUN_ID:-}"
GITHUB_DEPLOY_RUN_NUMBER="${GITHUB_DEPLOY_RUN_NUMBER:-}"
GITHUB_DEPLOY_RUN_ATTEMPT="${GITHUB_DEPLOY_RUN_ATTEMPT:-}"
GITHUB_DEPLOY_WORKFLOW="${GITHUB_DEPLOY_WORKFLOW:-Deploy Production}"
GITHUB_DEPLOY_EVENT="${GITHUB_DEPLOY_EVENT:-}"
GITHUB_DEPLOY_SERVER_URL="${GITHUB_DEPLOY_SERVER_URL:-https://github.com}"
GITHUB_DEPLOY_REPOSITORY="${GITHUB_DEPLOY_REPOSITORY:-hey-vera/heyvera}"
GITHUB_DEPLOY_ACTOR="${GITHUB_DEPLOY_ACTOR:-}"
GITHUB_DEPLOY_SHA="${GITHUB_DEPLOY_SHA:-$DEPLOY_COMMIT}"
GITHUB_DEPLOY_RUN_URL=""
if [ -n "$GITHUB_DEPLOY_RUN_ID" ] && [ -n "$GITHUB_DEPLOY_REPOSITORY" ]; then
  GITHUB_DEPLOY_RUN_URL="${GITHUB_DEPLOY_SERVER_URL}/${GITHUB_DEPLOY_REPOSITORY}/actions/runs/${GITHUB_DEPLOY_RUN_ID}"
fi
...
cat > "$DEPLOY_META_FILE" <<EOF
{
  ...
  "githubActions": {
    ...
    "runId": "${GITHUB_DEPLOY_RUN_ID}",
    "runNumber": "${GITHUB_DEPLOY_RUN_NUMBER}",
    "runAttempt": "${GITHUB_DEPLOY_RUN_ATTEMPT}",
    "event": "${GITHUB_DEPLOY_EVENT}",
    "repository": "${GITHUB_DEPLOY_REPOSITORY}",
    "actor": "${GITHUB_DEPLOY_ACTOR}",
    "headBranch": "${TARGET_BRANCH}",
    "headSha": "${GITHUB_DEPLOY_SHA}",
    "htmlUrl": "${GITHUB_DEPLOY_RUN_URL}"
  }
}
EOF
