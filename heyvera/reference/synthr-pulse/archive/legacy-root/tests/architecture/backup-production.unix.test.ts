...
describe.skipIf(isWindows)("production backup posture (bash/exec parts)", () => {
  // bash/exec tests isolated to this file; main file has only cross-platform or static tests.
  // skipIf provides structural skip on win (no early return/if in main, no fake pass its, no placeholders).
