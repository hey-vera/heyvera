...
internal/
  ...
  triage/             # unsorted idea intake when owner/stage/next action is unclear
...
3. `triage/` is for unsorted idea intake.
   Use it only when an idea's repo owner, stage, confidence, or next action is unclear. The goal is to preserve the idea without creating implementation pressure.

4. `archive/` is for retired internal documents.
...
5. `archive-artifacts/` is for non-canonical leftovers.
...
2. If the owner or next action is unclear, put it in `internal/triage/` and add it to `internal/idea-triage-index.md`.
3. Promote to `internal/active/` when it becomes a real workstream.
4. Write a canonical proposal in `docs/proposals/` if the work now has repo-level meaning.
5. Write an ADR in `docs/decisions/` if a structural decision is accepted.
6. Move superseded internal material to `internal/archive/`.
...
1. `idea-triage-index.md`
2. `active/idea-to-production-loop.md`
3. `active/golden-plan.md`
4. `active/revenue-architecture.md`
5. `active/roadmap.md`
6. Then task-specific files
