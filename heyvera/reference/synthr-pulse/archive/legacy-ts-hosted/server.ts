...
import { createPulseAgentApp } from './pulse-agent-mount.js'
import {
  isPinLocked,
  listAuditEvents,
...
// Use shared mount helper so server and tests use identical skip + router wiring.
const pulseAgentApp = createPulseAgentApp();
app.route('/', pulseAgentApp)  // the helper already does /v1/pulse mount + conditional skip

// ─── Auth Middleware ─────────────────────────────────────────────────────────
// Skip apiKeyAuth for pure no-key x402 intel/goal paths (robust, not fragile after root mount).
  ...
  return await withTenantContext(tenant.id, async () => {
    ...
    for (const link of links) {
      try {
        ...
        // Actualize GitHub -> unified gateway for semantic 10/10 intel (X + GitHub together)
        const { pushGitHubToIntelGateway } = await import('./github.js')
        await pushGitHubToIntelGateway(tenant.id, snapshot)
        db.upsertGitHubRepoLink(tenant.id, {
          repoId: link.repo_id,
          owner: link.owner,
          name: link.name,
...
// Proxy / expose knowledge gateway for frontend + unified intel (trustMode respected via metadata)
app.post('/api/intel/knowledge/upsert', async (c) => {
  const tenant = c.get('tenant')
  const body = await c.req.json()
  await withTenantContext(tenant.id, async () => {
    const { upsertKnowledgeToGateway } = await import('../src/core/x-intel-gateway.js')
    upsertKnowledgeToGateway(Array.isArray(body) ? body : [body])
  })
  return c.json({ ok: true })
})

app.post('/api/intel/knowledge/search', async (c) => {
  const tenant = c.get('tenant')
  const { query, limit = 5 } = await c.req.json()
  const result = await withTenantContext(tenant.id, async () => {
    const { searchKnowledge } = await import('../src/core/x-intel-gateway.js')
    return searchKnowledge(tenant.id, query, limit)
  })
