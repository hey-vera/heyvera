...
`;
    const report = evaluateProductionReadiness({}, {
      productionDeployWorkflow: badWorkflow,
    });
    const codes = (report.issues || []).map((i: any) => i.code);
    expect(codes).toContain("UNSAFE_PRODUCTION_DEPLOY_BRANCH_INPUT");
  });
